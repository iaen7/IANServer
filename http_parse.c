        ...
        switch(state){
            case sw_start:
                ...
                if(ch == CR || ch == LF)
                    break;
                if((ch < 'A' || ch > 'Z') && ch != '_')
                    return IAN_HTTP_PARSE_INVALID_METHOD;
                state = sw_method;
                break;
            case sw_method:
                if(ch == ' '){
                    ...
                    switch(p-m){
                        case 3:
                            if(ian_str3_cmp(m, 'G', 'E', 'T', ' ')){
                                request->method = IAN_HTTP_GET;
                                break;
                            }
                        case 4:
                            if(ian_str3Ocmp(m, 'P', 'O', 'S', 'T')){
                                request->method = IAN_HTTP_POST;
                                break;
                            }
                            if(ian_str4cmp(m,'H','E','A','D')){
                                request->method = IAN_HTTP_HEAD;
                                break;
                            }
                            break;
                        default:
                            request->method = IAN_HTTP_UNKNOWN;
                            break;
                    }
                    state = sw_spaces_befor_uri;
                    break;
                }
            ...
            case sw_spaces_before_uri:
                ...
                switch(ch){
                    case ' ':
                        break;
                    default:
                        return IAN_HTTP_PARSE_INVALID_REQUEST;
                }
                break;
    ...
    for(pi = request->pos; pi<request->last; pi++){
        ...
        switch(state){
            ...
            case sw_cr:
                if(ch == LF){
                    state = sw_crlf;
                    hd = (ian_http_header_t *)malloc(sizeof(ian_http_header_t));
                    ...
                    return IAN_HTTP_PARSE_INVALID_HEADER;
            case sw_crlf:
                if(ch == CR)
                    state = sw_crlfcr;
                else{
